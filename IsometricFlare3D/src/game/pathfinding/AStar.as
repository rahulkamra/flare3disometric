...
//		private var _heuristic:Function = euclidian;
		...
		public function findPath(grid:Grid):Boolean
		{
			...
			var canGo:Boolean = search();
			var end:Number = new Date().time;
			//trace("Time for pathFinding is "  , (end-start)/1000 , " Start Node is " , _startNode.x," ",_startNode.y , " End Node is " , _endNode.x," ",_endNode.y)
			return canGo;
		}
		...
		public function search():Boolean
		{
			...
			while(node != _endNode)
			{
				...
				var endX:int = Math.min(_grid.getNumCols() - 1, node.x + 1);
				var startY:int = Math.max(0, node.y - 1);
				var endY:int = Math.min(_grid.getNumRows() - 1, node.y + 1);
